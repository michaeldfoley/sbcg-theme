...
<?php if( apply_filters('em_booking_form_show_register_form',true) ): ?>
	...
	<?php else: ?>
	<p>
    <strong><?php _e('Name','dbem') ?>:</strong> <?php echo esc_attr($current_user->display_name); ?><br />
    <strong><?php _e('E-mail','dbem') ?>:</strong> <?php echo esc_attr($current_user->user_email); ?>
	</p>
	<?php endif; ?> 
	<p>
  ...
  <p>
  	<label for='booking_comment'><?php _e('Notes', 'dbem') ?></label>
